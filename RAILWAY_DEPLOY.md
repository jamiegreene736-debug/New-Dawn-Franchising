...
- Campaign email throttle (deliverability): drip emails are sent Mon–Fri 8 AM–5 PM CT, with organic jitter, per-domain pacing, and hourly + daily caps. Tune with the optional `EMAIL_DAILY_CAP` (default 100 × configured sender mailboxes — 400 when all four app passwords are set), `EMAIL_HOURLY_CAP` (default 50) and `EMAIL_DOMAIN_GAP_SECONDS` (default 45) variables. Caps are DB-backed, so they hold across redeploys. Sender rotation is on by default and spreads volume across every mailbox with a `GMAIL_APP_PASSWORD_*` secret, keeping each address near ~100/day.
