...
- `mobile_notification_preferences`, `mobile_push_devices`, `mobile_notification_reminders`, and `mobile_notifications` — user-controlled categories, revocable delivery endpoints, important dates, and an authenticated inbox with source-review metadata;
...
The generated staging package comprises `migrations/mobile/0000_mobile_identity_foundation.sql`, `migrations/mobile/0001_mobile_pathway_pilot.sql`, and `migrations/mobile/0002_mobile_notifications.sql`. Its static verifier permits only mobile-namespaced enum, table, foreign-key, and index creation; the existing `crm_clients` table may be referenced by foreign keys but cannot be altered. It rejects data writes, drops, executable SQL, core-table changes, unexpected objects, and incomplete packages. The reviewed package currently contains 94 statements across 17 mobile tables and 14 mobile enums, with aggregate SHA-256 `1d3855e0c9091d4ebba288304ca99913398e0f7d197a90b7f8206b7403e240d2`.

Run `npm run mobile:migration:verify` to validate the committed artifacts without connecting to a database. Run `npm run mobile:schema:readiness` only with an intentionally selected environment; it opens a read-only transaction, reports table presence only, and always rolls back. The identity and pathway migrations were transactionally rehearsed, rolled back, and then applied to the isolated Railway `staging` database on September 2, 2026. The additive notification migration was applied to that same isolated database on September 8, 2026 after exact-target preflight and transactional readback. No mobile migration has been applied to production.
