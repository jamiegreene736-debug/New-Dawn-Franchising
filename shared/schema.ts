...
export const outreachDailyPlans = pgTable("outreach_daily_plans", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  planDate: text("plan_date").notNull(), // YYYY-MM-DD (ET) — the day this plan covers
  status: text("status").default("awaiting_approval"), // awaiting_approval | approved | rejected | executing | completed | failed | expired
  ...
  searchQueries: jsonb("search_queries").$type<{
    query: string; source: "serpapi"|"seamless"|"apollo"|"hunter";
    purpose: string; executed?: boolean; resultsCount?: number; error?: string;
  }[]>(),
  ...
  reviewNotes: text("review_notes"),
  executedAt: timestamp("executed_at"),
  // Autopilot bookkeeping: set when the plan was auto-approved (no human tap),
  // how many execution attempts have run, and the last execution error (kept so
  // an unattended failure is diagnosable days later).
